...
def run_png_blob(
    ...
    ----------
    input_blob: str
        Path of storage blob contaning the segmentation masks in png file
        format.
    output_dir: Path | None, optional
        Directory, if any, to store converted segmentation instances.
...
def run_csv_blob(
    ...
    dfs = [
        pd.read_csv(
            ann_bucket.get_blob(ann_blob).open('rt'),
            index_col=0
        )
        for ann_blob in annotation_blobs
    ]
    ...
    # Store to bucket
    if output_bucket_obj is not None:
        logging.info("Writing annotations to output bucket.")
...
def convert_csv(
    ...
    if store_bucket:
        if output_bucket is None:
            date_str = (datetime.date.today())
            output_bucket = (
                f"panoptils-annotations-{date_str}"
            )
        output_bucket_obj = output_client.bucket(output_bucket)
    ...
    for ann_blob in ann_bucket.list_blobs(prefix=MANUAL_ANNOTATIONS_PREFIX):
        ...
        blob_name = ann_blob.name.split("/")[-1].replace(".csv", "")
        container_id = blob_name.split("_")[0]

        # Check whether the output blobs already exist, and skip if
        # they do
        if keep_existing:
            ann_blob_name = f"{container_id}_ann.dcm"
            ann_output_blob = output_bucket_obj.get_blob(ann_blob_name)
            if ann_output_blob.exists():
                continue
